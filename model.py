...
class ECGModel(torch.nn.Module):
    ...
    def __init__(self, cfg, num_input_channels, num_outputs, binary, score=None):
        ...
        if self.binary:
            ...
            self.recall_score = score or (lambda y, yh, loss: sklearn.metrics.recall_score(y, yh))
            self.specificity_score = score or (lambda y, yh, loss: sklearn.metrics.specificity_score(y, yh))
        ...
        print(f'model: {self.cfg["model_type"]}')
    ...
    def train_step(self, x, y):
        ...
        if torch.isnan(y_hat).any().item():
            nan_mask = torch.isnan(y_hat)
            y_hat[nan_mask] = torch.mean(y_hat[~nan_mask])
